...
class metaMobileData(Dataset):
    """
    The data processing might look a bit complicated but all you do is take a set of
    long trajectory and create training/validation data by splitting them into non-overlapping
    windows as mentioned in Appendix E of the ICLR 2022 paper. /https://arxiv.org/pdf/2206.14697.pdf

    There is also some procedures to calculate the difference to next state and calculate losses on this.
    """
    ...
    def get_statistics(self, data, dim, difference=False):
        """
        data: data for which normalization constants to be inferred
        dim: use only first "dim" dimensions
        difference: whether to predict differences to next state or not
        """
        if difference:
    ...
    def _get_batch(self, train=True, percentage_imputation=0.0):
        """
        train: if train dataset or not
        percentage imputation: how much of the data to impute
        """

        """
        Takes multiple paths and splits them into non overlapping windows based on random locations within a trajectory
        as mentioned in Appendix E of the ICLR 2022 Paper /https://arxiv.org/pdf/2206.14697.pdf
        """
